...
def optimize_arima(series, steps=1):
    ...
    Returns:
    - best_model: ARIMA model with best parameters.
    """
    def objective(trial):
...
def forecast_arima(series, steps=1, optimize=False):
    """
    Forecast using ARIMA model, with an option to optimize parameters.
    
    Parameters:
    - series: pd.Series, the time series data.
    - steps: int, number of periods to forecast.
    - optimize: bool, whether to use Optuna to optimize parameters.
...
def optimize_sarima(series, steps=1):
    """
    Optimize SARIMA model using Optuna.
    ...
    Returns:
    - best_model: SARIMAX model with best parameters.
    """
    def objective(trial):
        p = trial.suggest_int('p', 0, 5)
        d = trial.suggest_int('d', 0, 2)
...
def forecast_sarima(series, steps=1, optimize=False):
    """
    Forecast using SARIMA model, with an option to optimize parameters.
...
def optimize_exponential_smoothing(series, steps=1):
    """
    Optimize Exponential Smoothing model using Optuna.
    ...
    Returns:
    - best_model: Exponential Smoothing model with best parameters.
    """
    def objective(trial):
...
def forecast_exponential_smoothing(series, steps=1, optimize=False):
    """
    Forecast using Exponential Smoothing model, with an option to optimize parameters.
